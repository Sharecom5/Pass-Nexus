...
export async function GET(req: NextRequest, props: { params: Promise<{ passId: string }> }) {
  try {
    const { passId } = await props.params;
    await connectDB();
...
export async function POST(req: NextRequest, props: { params: Promise<{ passId: string }> }) {
  try {
    const { passId } = await props.params;
